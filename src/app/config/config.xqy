...
declare variable $c:ROXY-OPTIONS :=
  <options>
    <layouts>
      <layout format="html">smiths</layout>
    </layouts>
  </options>;
...
declare variable $c:ROXY-ROUTES :=
  <routes xmlns="http://marklogic.com/appservices/rest">
	<request uri="^/(img)/(.*)" endpoint="/roxy/query-router.xqy">
	      <uri-param name="controller" default="img">$1</uri-param>
	      <uri-param name="uri">$2</uri-param>
	      
	      <http method="GET"/>
	      
	</request>
	<request uri="^/image-content/(.*)$" endpoint="/app/controllers/img.xqy">
      <uri-param name="uri">$1</uri-param>
      <http method="GET"/>
      <http method="HEAD"/>
    </request>
		<request uri="^/search.xml/(.*)$" endpoint="/roxy/query-router.xqy">
      <uri-param name="q">$1</uri-param>
	  <uri-param name="page">$2</uri-param>
      <http method="GET"/>
      <http method="HEAD"/>
    </request>
...
declare variable $c:SEARCH-OPTIONS :=
  <options xmlns="http://marklogic.com/appservices/search">
	...
	<searchable-expression>fn:collection("master")</searchable-expression>
    <term>
