...
declare function i:create-metamapping() as element(i:meta-mappings)* {
    ...
    <i:meta-mappings>
        <i:use-meta-mapping order="1">/mappings/Default-ONIX3.0.xml</i:use-meta-mapping> 
        <i:use-meta-mapping order="2">/mappings/pearson-to-onix.xml</i:use-meta-mapping>
    </i:meta-mappings>
...
declare function i:create-groups($file) as element(i:groups)* { 
  ...
  let $grouped-out :=
    ...
    for $fn in xdmp:zip-manifest($file)//zip:part/text()
        ...
        if ($file-extn = ('pdf','epub','xml')) then
            ...
            (: MW: Insert document :)
            let $docout := xdmp:document-insert(fn:concat("/", $group-name, "/", $file-uri), $filecontent)
            
            (: MW: Validate document :)
            let $schema-err :=
            try {
                validate strict {$filecontent}
            } 
